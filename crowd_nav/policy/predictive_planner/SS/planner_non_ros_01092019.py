#!/usr/bin/env python
"""
Subscribe to rostopic od tracked Objects
Pubslish list of waypoints

Uses a Generative RNN and MCTS for path planning in dynamic environments

#todo:
update input and output formats (create ros msg format, rather than using multiarray)
test pipeline
retrain
...
#import roslib
#roslib.load_manifest('animal_tracking')
import sys
#import rospy
import numpy as np
import time
...
#from predictive_planner.msg import DetectionMsg
#from predictive_planner.msg import ObjectInfo
import copy
...
#from mcts_par import createRootNode, searchUCT_par, bestChildPar
...
class Planner(object):
    ...
    def __init__(self, model_path, lookahead, stdev=None, use_cv=False, use_sef2=False):
        ...
        #self.det_sub_topic = arg_detin
        #self.goal_sub_topic = arg_goal
        #self.path_pub_topic = arg_path

        # ROS
        #self.goal_sub = rospy.Subscriber(self.goal_sub_topic, Float32MultiArray, self.goal_callback)
        #self.det_sub = rospy.Subscriber(self.det_sub_topic, Float32MultiArray, self.det_callback)
        #self.path_pub = rospy.Publisher(self.path_pub_topic, Float32MultiArray, queue_size=1)
    ...
    def updater(self):
        """
        constantly runs, checking if new msgs have arrived

        """   
        print("Awaiting detections...") 
        while self.runthread:            
            #self.get_display_image()
            if(self.det_updated):
                print("encoding observation")
                self.det_updated = False
                #self.obs_current contains positions of all agents for last observation period as well as actions (future positions) of robot for same timesteps.
                # we assume that all tracks have been filtered and normalised (as per the training dataset distribution)
                # sort out input msg to get X, R and num_agents
                X, R, offset = self.convertInput(self.obs_current, self.NUM_PARS)
                next_path, next_actions = self.planPath(X,R,offset)
                print(next_path, next_actions)
                #publish new path as waypoints
                self.path_msg.data = next_path
    ...
    def det_callback(self, msg):
        ...
        self.det_updated = True
        raw_data = msg.data
        chunk_len = (self.OBS_WINDOW + 2 ) * self.NUM_DIMS #break up data into chunks (per agent). Extra for first robot offset, and for current observation (used in R input to model)
        chunked_data = [raw_data[i:i + chunk_len] for i in range(0, len(raw_data), chunk_len)]
        for i in range(len(chunked_data)):
            chunked_data[i] = [chunked_data[i][j:j + self.NUM_DIMS] for j in range(0, len(chunked_data[i]), self.NUM_DIMS)]

        assert len(chunked_data[-1]) == (self.OBS_WINDOW + 2 ) , "Check Incoming agent window length {} and expected {}".format(len(raw_data),chunk_len)

        self.obs_current = chunked_data #get observed tracks and robot positions
        self.num_agents = len(chunked_data) -1 #first dimension is robot 
